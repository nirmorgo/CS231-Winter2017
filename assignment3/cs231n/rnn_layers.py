...
def rnn_step_backward(dnext_h, cache):
    ...
    # forward pass:  next_h = np.tanh(prev_h.dot(Wh) + x.dot(Wx) + b) 
    
    dactivation = (1 - next_h*next_h) * dnext_h
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        dxi, dh_prev, dWxi, dWhi, dbi  = rnn_step_backward(dh_current, cache[i])
        dx[:,i,:] += dxi
        dWx += dWxi
        dWh += dWhi
        db += dbi
